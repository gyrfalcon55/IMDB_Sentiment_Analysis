...
class input_preprocessing:
    ...
    def preprocessing_input(self,user_input):

        self.user_input = user_input
                
                
        log.info("preprocessing user_input and converting text to vector")
        self.new_output = re.sub(r'[^a-zA-Z0-9\s]', '', self.user_input)
        self.new_output = self.preprocess.clean_text(self.new_output)
        self.text_vector = self.vectorizer.transform([self.new_output])
        return self.text_vector
class Model_prediction(input_preprocessing):

    def prediction(self, user_input):
        log.info("Model prediction on user_input")
        self.vector = self.preprocessing_input(user_input)
        self.result = self.model.predict(self.vector)

        sentiment = "Positive" if self.result[0] == 1 else "Negative"
        log.info(f"Predicted Sentiment: {sentiment}")
        return sentiment    
